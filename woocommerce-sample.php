		...
		if ($is_sample){
			...
			foreach ($cart_items as $cart_id_key => $cart_item){
				if ($cart_item['unique_key'] == $unique_key){
					...
					wc_add_notice( __( 'A sample of the same product is already present into your cart', 'woosample' ), $notice_type = 'error' );
					return false;
				}
				if ($cart_item['product_id'] == $product_id){
					...
					wc_add_notice( __( 'You have already added this product on your cart, you can\'t add a sample of the same item', 'woosample' ), $notice_type = 'error' );
					return false;
				}
			}
		}
